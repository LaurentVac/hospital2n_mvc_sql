...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $patient = $profilPatient->profilPatient($id);
        ...
        if(empty($errorsArray)&& $_SERVER['REQUEST_METHOD'] == 'POST' ){
            $modifPatient1 = new Patient ($name, $firstname, $birthDate, $phone, $mail, $id);
            $modifPatient = $modifPatient1->updatePatient();
            if($modifPatient === true){
            
            header('location: /controllers/list-patientsCtrl.php');
            }else{
                $errorsArray['error_global'] = "La mise à jour n'a pas était effectuée";
            }
        }else{
            $error = "La modification n'as pas était enregistrée";
        }
    ...
    include(dirname(__FILE__).'/../views/modif_patient.php');
